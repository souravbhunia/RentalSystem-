...
//                user.setPhoneNumber(rs.getString("phoneNumber"));
    ...
    public List reportOrderCustomer(int userId) throws SQLException, Exception {
        ...
        try {
            String sql = "SELECT products.productName, products.productMake, products.productPrice, products.productSpecification  FROM orders, products where orders.userId=?";
            con = ConnectionManager.getConnection();
            System.out.println("Connection is " + con);
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, userId);
